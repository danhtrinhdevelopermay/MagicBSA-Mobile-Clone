...
class EnhancedMaskPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawImageRect(originalImage, srcRect, imageRect, Paint());

    // Draw mask strokes with gradient animation
    for (int strokeIndex = 0; strokeIndex < maskStrokes.length; strokeIndex++) {
      final stroke = maskStrokes[strokeIndex];
      ...
      for (int pointIndex = 0; pointIndex < stroke.length; pointIndex++) {
        ...
        final gradientPosition = (animationValue + (strokeIndex * 0.1) + (pointIndex * 0.01)) % 1.0;
        Color strokeColor;
        
        if (gradientPosition < 0.33) {
          // Red to Orange
          final t = gradientPosition / 0.33;
          ...
          // Orange to Green
          final t = (gradientPosition - 0.33) / 0.33;
          strokeColor = Color.lerp(
            const Color(0xFFFF9800), // Orange
          ...
          // Green to Red
          final t = (gradientPosition - 0.66) / 0.34;
          strokeColor = Color.lerp(
            const Color(0xFF4CAF50), // Green
            const Color(0xFFFF5722), // Red
            t,
        ...
        // Draw brush stroke with animated size if processing
        final currentBrushSize = isProcessing 
            ? brushSize * (1.0 + 0.1 * sin(animationValue * 8 * pi))
            : brushSize;

        final strokePaint = Paint()
          ..color = strokeColor.withOpacity(brushOpacity)
          ..style = PaintingStyle.fill;

        canvas.drawCircle(adjustedPoint, currentBrushSize / 2, strokePaint);

        // Draw animated white border
        if (isProcessing) {
          final borderOpacity = (sin(animationValue * 2 * pi) + 1) / 4 + 0.2; // 0.2 to 0.7
          final borderPaint = Paint()
            ..color = Colors.white.withOpacity(borderOpacity)
            ..style = PaintingStyle.stroke
            ..strokeWidth = 2;

          canvas.drawCircle(adjustedPoint, currentBrushSize / 2 + 1, borderPaint);
        }
      }
    }
  }
